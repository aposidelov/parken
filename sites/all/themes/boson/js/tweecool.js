		...
		tweecool : function(options) {
			...
			function xTimeAgo(time) {
				...
				if (timeDiff > second && timeDiff < minute) {
                                    return Math.round(timeDiff / second) + " sekund"+(Math.round(timeDiff / second) > 1?'er':'')+" siden";
				} else if (timeDiff >= minute && timeDiff < hour) {
					return Math.round(timeDiff / minute) + " minut"+(Math.round(timeDiff / minute) > 1?'ter':'')+" siden";
				} else if (timeDiff >= hour && timeDiff < day) {
					return Math.round(timeDiff / hour) + " time"+(Math.round(timeDiff / hour) > 1?'r':'' )+" siden";
				} else if (timeDiff >= day && timeDiff < week) {
					return Math.round(timeDiff / day) + " dag"+(Math.round(timeDiff / day) > 1 ?'e':'')+" siden";
				} else if (timeDiff >= week && timeDiff < month) {
					return Math.round(timeDiff / week) + " uge"+(Math.round(timeDiff / week) > 1?'r':'')+" siden";
				} else if (timeDiff >= month && timeDiff < year) {
					return Math.round(timeDiff / month) + " måned"+(Math.round(timeDiff / month) > 1 ?'er':'')+" siden";
				} else {
			...
			return this.each(function() {
				...
				$.getJSON("https://www.api.tweecool.com/?screenname=" + o.username + "&count=" + o.limit, function(data) {
					...
					if (data.errors || data == null) {
						wrapper.html('Ingen tweets tilgængelige.');
						return false;
					}
